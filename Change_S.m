...
% When S=[0.37 1.37 2.37 3.37 4.37],reverse solution of Laplace space pseudo-bottomhole pressure (equation (47)) to real space pseudo-bottomhole pressure (equation (48)) 
for k=1:5
    for i=1:L
        ft(i,k)=0;
        u=log(2)/tD(i);
        for j=1:M
            ...
            fz1=G1(rD,RD,z,alpha1,alpha2,alpha3,alpha4,omega,lambda)-S(k);
            % Code for denominator of equation(47)
            fm1=CD.*z.*G1(1,RD,z,alpha1,alpha2,alpha3,alpha4,omega,lambda)-(1+S(k).*CD.*z);
...
% Three point method for derivative calculation (pseudo-bottomhole pressure derivative),when S=[0.37 1.37 2.37 3.37 4.37];
for k=1:5
...
% Double logarithmic curves of pseudo-bottomhole pressure at different S
...
% Double logarithmic curves of pseudo-bottomhole pressure derivatives at different S
...
legend('\itS=0.37','\itS=1.37','\itS=2.37','\itS=3.37','\itS=4.37')
