...
% When RD=[500 1000 1500 2000 2500],reverse solution of Laplace space pseudo-bottomhole pressure (equation (47)) to real space pseudo-bottomhole pressure (equation (48)) 
for k=1:5
    for i=1:L
        ft(i,k)=0;
        u=log(2)/tD(i);
        for j=1:M
            ...
            fz1=G1(rD,RD(k),z,alpha1,alpha2,alpha3,alpha4,omega,lambda)-S;
            % Code for denominator of equation(47)
            fm1=CD.*z.*G1(1,RD(k),z,alpha1,alpha2,alpha3,alpha4,omega,lambda)-(1+S.*CD.*z);
...
% Three point method for derivative calculation (pseudo-bottomhole pressure derivative),when RD=[500 1000 1500 2000 2500];
for k=1:5
...
% Double logarithmic curves of pseudo-bottomhole pressure at different RD
...
hold on
% Double logarithmic curves of pseudo-bottomhole pressure derivatives at different RD
...
legend('\itR_{D}=500','\itR_{D}=1000','\itR_{D}=1500','\itR_{D}=2000','\itR_{D}=2500')
