...
alpha4=0;
omega=0.005;
lambda=[ 10^(-10) 10^(-9) 10^(-8) 10^(-7)  10^(-6) ];
CD=60 ;
S=2.37;
tD = zeros(1,200);
L = length(tD);
...
% When lambda=[ 10^(-10) 10^(-9) 10^(-8) 10^(-7)  10^(-6) ],reverse solution of Laplace space pseudo-bottomhole pressure (equation (47)) to real space ...
...
for k=1:5
    for i=1:L
        ...
        for j=1:M
            ...
            fz1=G1(rD,RD,z,alpha1,alpha2,alpha3,alpha4,omega,lambda(k))-S;
            % Code for denominator of equation(47)
            fm1=CD.*z.*G1(1,RD,z,alpha1,alpha2,alpha3,alpha4,omega,lambda(k))-(1+S.*CD.*z);
...
% Three point method for derivative calculation (pseudo-bottomhole pressure derivative),when lambda=[ 10^(-10) 10^(-9) 10^(-8) 10^(-7)  10^(-6) ];
for k=1:5
...
% Double logarithmic curves of pseudo-bottomhole pressure at different lambda
...
hold on
loglog(tD,y(:,5),'Color',[0, 0, 1], 'Linewidth',1.5)
hold on
% Double logarithmic curves of pseudo-bottomhole pressure derivatives at different lambda
...
hold on
loglog(tD,D_y(:,5),'--','Color',[0, 0, 1], 'Linewidth',1.5)
hold on
...
legend('\it\lambda= 10^{-10}','\it\lambda= 10^{-9}','\it\lambda= 10^{-8}','\it\lambda= 10^{-7}','\it\lambda= 10^{-6}')
