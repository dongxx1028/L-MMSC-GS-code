...
% When CD=[10 35 60 85 110],reverse solution of Laplace space pseudo-bottomhole pressure (equation (47)) to real space pseudo-bottomhole pressure (equation (48)) 
for k=1:5
    for i=1:L
        ft(i,k)=0;
        u=log(2)/tD(i);
        for j=1:M
            ...
            fm1=CD(k).*z.*G1(1,RD,z,alpha1,alpha2,alpha3,alpha4,omega,lambda)-(1+S.*CD(k).*z);
...
% Three point method for derivative calculation (pseudo-bottomhole pressure derivative),when lambda=CD=[10 35 60 85 110];
for k=1:5
...
% Double logarithmic curves of pseudo-bottomhole pressure at different CD
...
hold on
% Double logarithmic curves of pseudo-bottomhole pressure derivatives at different CD
...
legend('\itC_{D}=10','\itC_{D}=35','\itC_{D}=60','\itC_{D}=85','\itC_{D}=110')
